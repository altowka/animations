var sketchProc = function (processingInstance) {
    with(processingInstance) {
        ...
        for ( var i = 0; i<4; i++) {//to, co nizej tylko fajniej:)
            var rolX = -43 + (i*53);
        ellipse (rolPlace + rolX, 310, 50, 50);
            
        }
        /*ellipse(rolPlace-43,310,50,50);
        ellipse(rolPlace+10,310,50,50);
        ellipse(rolPlace+63,310,50,50);
        ellipse(rolPlace+116,310,50,50);*/
        strokeWeight(2);
        rect(rolPlace+63,175,75,53);
        
         if (rolPlace > 256) {   //dodana konstrukcja if i zmienna speed. wczeniej było
                                 //po prostu ,,rolPlace --;''- rolka przejazdała i znikała
                speed = -4;
            }
            if (rolPlace < 65) {
                speed = 4;
